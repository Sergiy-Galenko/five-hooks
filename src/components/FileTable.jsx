...
const FileTable = () => {
  const [data, setData] = useState([
    {
      ...
      children: [
        ...
        { key: "1-2", name: "File 1-2", type: "File", size: "20 KB" },
        {
          ...
          isFolder: true,
          children: [
            { key: "1-3-1", name: "File 1-3-1", type: "File", size: "10 KB" },
          ],
        },
      ],
    },
  ...
  const handleDelete = () => {
    if (selectedKeys.length === 0) {
      messageApi.warning("No file or folder selected for deletion");
      return;
    }
    ...
    const deleteRecursively = (items) =>
      items
        ...
        .map((item) => ({
          ...item,
          children: item.children ? deleteRecursively(item.children) : undefined,
        }));

    setData(deleteRecursively(data));
    messageApi.success(`${selectedKeys.length} item(s) deleted successfully`);
    setSelectedKeys([]);
  };
